...
def train(
    ...
    for epoch in trange_epochs:
        ...
        for controls, states, targets, dts in tqdm(train_loader, disable=not verbose, desc="Train", leave=False):
            ...
            loss = criterion(pred_x, true_x) + criterion(pred_y, true_y) + criterion(pred_theta, true_theta)
            ...
            with torch.no_grad():
                if model_baseline is not None:
                    ...
                    baseline_predictions = torch.cat(baseline_predictions).view(-1, *baseline_predictions[0].shape)

                    base_pred_x, base_pred_y, base_pred_theta = unroll_world_frame_torch(baseline_predictions, dts_transpose)
                    baseline_loss = criterion(base_pred_x, true_x) + criterion(base_pred_y, true_y) + criterion(base_pred_theta, true_theta)
